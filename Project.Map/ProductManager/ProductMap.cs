...
namespace Project.Map.ProductManager
{
    public class ProductMap : BaseMap<ProductEntity, int>
    {
        public ProductMap() : base("PRM_Product")
        {
            ...
            Map(p => p.ProductCode);
            Map(p => p.Unit);
            Map(p => p.BriefDescription).CustomSqlType("text").CustomType("StringClob");
            Map(p => p.Description).CustomSqlType("text").CustomType("StringClob");
            Map(p => p.Weight);
            Map(p => p.WeightUnit);
            Map(p => p.MarketPrice);
            Map(p => p.SellPrice);
            ...
            Map(p => p.IsPromotion);
            Map(p => p.PromotionPriceArea);
            Map(p => p.ImageUrl);


            HasMany(p => p.GoodsEntityList)
 .AsSet()
 .LazyLoad()
 .Cascade.All().Inverse()
 .KeyColumn("ProductId");
            ...
            HasMany(p => p.GoodsSpecValueEntityList)
 .AsSet()
 .LazyLoad()
 .Cascade.All().Inverse()
 .KeyColumn("ProductId");
...
.KeyColumn("ProductId");
            ...
            HasMany(p => p.ProductImageEntityList)
.AsSet()
.LazyLoad()
.Cascade.All().Inverse()
.KeyColumn("ProductId");
