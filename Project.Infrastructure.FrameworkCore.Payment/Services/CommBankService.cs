    ...
    internal class CommBankService : BaseService, IPayService 
    {
        ...
        public PayResult CheckNotifyData(NotifyEnum notifyType)
        {
            ...
            try
            {
                ...
                {
                    result.Status = false;
                   LoggerHelper.InfoFormat(LogType.PaymentLogger,"交通银行返回商户验签失败,返回码:{0},错误信息:{1},返回详细信息:{2}。\r\n", retCode, errMsg, notifyMsg);
                }
                else
                {
               ...
               LoggerHelper.InfoFormat(LogType.PaymentLogger,"交通银行支付通知:{0},银行返回信息{1}。\r\n", result.OrderNo, notifyMsg);
                return result;
            }
            catch (Exception ex)
            {
                LoggerHelper.InfoFormat(LogType.PaymentLogger, "交通银行通知,错误异常信息{0}。\r\n", ex.Message);
                return result;
            }
        }
        ...
        private Tuple<string, string> GetResponseData(string sourceMsg)
        {
            ...
            if (!retCode.Equals("0"))
            {
                LoggerHelper.InfoFormat(LogType.PaymentLogger, "交通银行链接,返回码:{0},错误信息:{1},详细信息:{2}\r\n", retCode, errMsg, sourceMsg);
            }
