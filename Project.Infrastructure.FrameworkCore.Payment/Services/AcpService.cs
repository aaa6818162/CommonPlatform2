...
namespace Project.Infrastructure.FrameworkCore.Payment.Services
{
    public class AcpService 
    {
        ...
        public static void Sign(Dictionary<string, string> reqData, Encoding encoding, string certPath, string certPwd)
        {
            ...
            LoggerHelper.Info(LogType.PaymentLogger,"待签名排序串：[" + stringData + "]");
            ...
            LoggerHelper.Info(LogType.PaymentLogger, "sha1结果：[" + stringSignDigest + "]");
            ...
            LoggerHelper.Info(LogType.PaymentLogger, "签名结果：[" + stringSign + "]");
        ...
        public static bool Validate(Dictionary<string, string> rspData, Encoding encoding)
        {
            ...
            LoggerHelper.Info(LogType.PaymentLogger, "签名原文：[" + signValue + "]");
           ...
           LoggerHelper.Info(LogType.PaymentLogger, "排序串：[" + stringData + "]");
            byte[] signDigest = SecurityUtil.Sha1X16(stringData, encoding);
            string stringSignDigest = BitConverter.ToString(signDigest).Replace("-", "").ToLower();
           LoggerHelper.Info(LogType.PaymentLogger, "sha1结果：[" + stringSignDigest + "]");
            ...
            if (null == provider)
            {
                LoggerHelper.Error("未找到证书，无法验签，验签失败。");
                return false;
            }
            bool result = SecurityUtil.ValidateBySoft(provider, signByte, encoding.GetBytes(stringSignDigest));
            if (result)
            {
               LoggerHelper.Info(LogType.PaymentLogger, "验签成功");
            }
            else
            {
               LoggerHelper.Info(LogType.PaymentLogger, "验签失败");
            }
            return result;
        }
        ...
        {
           LoggerHelper.Info(LogType.PaymentLogger, "控件返回报文验签：[" + jsonData + "]");
            //获取签名
        ...
        public static string CreateAutoFormHtml(string reqUrl, Dictionary<string, string> reqData, Encoding encoding)
        {
           ...
           LoggerHelper.Info(LogType.PaymentLogger, "生成自动跳转的HTML：[" + result + "]");
            return result;
        }
        ...
        public static Dictionary<String, String> Post(Dictionary<String, String> reqData, String reqUrl, Encoding encoding)
        {
            ...
            try
            {
               ...
               LoggerHelper.Info(LogType.PaymentLogger, "发送post请求，url=[" + reqUrl + "]，data=[" + postData + "]");
                ...
                {
                   LoggerHelper.Info(LogType.PaymentLogger, "收到后台应答，data=[" + sResult + "]");
                ...
                else
                {
                   ...
                   LoggerHelper.Info(LogType.PaymentLogger, "非200HTTP状态，httpStatus=" + httpStatus + "，data=[" + sResult + "]");
                    return new Dictionary<string, string>();
                }
            }
            catch (Exception ex)
            {
                LoggerHelper.Error("post失败，异常：" + ex.Message);
                return new Dictionary<string, string>();
            }
        ...
        {
           LoggerHelper.Info(LogType.PaymentLogger, "发送get请求：" + reqUrl); //get先debug，缴费相关前端接口数据量貌似还挺大
            try
            {
                if ("false".Equals(SDKConfig.IfValidateRemoteCert)) //测试环境不验https证书
                    System.Net.ServicePointManager.ServerCertificateValidationCallback = new RemoteCertificateValidationCallback(ValidateServerCertificate);
            ...
            {
                LoggerHelper.Error("get失败，异常：" + ex.Message);
                return ex.Message;
            }
        ...
        public static string EnCodeFileContent(string filePath, Encoding encoding)
        {
            ...
            {
                LoggerHelper.Error(filePath + "文件不存在，无法得到fileContent");
                return null;
            }
        }
        ...
        public static bool DeCodeFileContent(Dictionary<string, string> resData, string fileDirectory)
        {
            ...
            {
                LoggerHelper.Error("保存fileContent出错：" + e.Message);
                return false;
            }
        }
        ...
        public static Dictionary<string, string> ParseCustomerInfo(string customerInfoStr, Encoding encoding, string certPath, string certPwd)
        {
            ...
            {
                LoggerHelper.Error("customerInfoStr解析失败，异常：" + e.Message);
                return new Dictionary<string, string>();
            }
