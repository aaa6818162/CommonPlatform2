...
//选择性别
jQuery.validator.addMethod("checkGender", function (value, element, param) {
    var gender = $("input[name='" + element.name + "']:checked").val();
    if (gender == undefined) {
        return false;
    }
    return true;
...
//验证长度包含空格
jQuery.validator.addMethod("rangeFullLength", function (value, element, param) {
    var length = value.length;
    return (length >= param[0] && length <= param[1]);
}, $.validator.format("请确保输入的值在{0}-{1}个字节之间"));

//验证必选项包含空格
jQuery.validator.addMethod("required1", function (value, element, param) {
    var length = value.length;
    return length > 0;
}, "必输字段");
