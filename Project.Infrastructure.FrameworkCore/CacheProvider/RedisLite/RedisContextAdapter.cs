...
namespace Project.Infrastructure.FrameworkCore.CacheProvider.RedisLite
{
    ...
    /// Redis操作
    /// </summary>
    public class RedisContextAdapter
    {
        ...
        public RedisContextAdapter(string host, int port)
        {
            ...
            RedisSession.Configuration(this._config);
            _connection = RedisSession.Instance;
        }
        ...
        public RedisContextAdapter(RedisConfig config)
        {
            this._config = config;
            RedisSession.Configuration(this._config);
            _connection = RedisSession.Instance;
        ...
        public bool StringSet(List<KeyValuePair<RedisKey, RedisValue>> keyValues)
        {
            List<KeyValuePair<RedisKey, RedisValue>> newkeyValues =
                keyValues.Select(p => new KeyValuePair<RedisKey, RedisValue>(AppendPrefix(p.Key), p.Value)).ToList();
        ...
        {
            List<string> newKeys = listKey.Select(AppendPrefix).ToList();
        ...
        {
            key = AppendPrefix(key);
            return Do(redis => redis.SetAdd(key, value));
        }
        ...
        {
            List<string> newKeys = keys.Select(AppendPrefix).ToList();
        ...
        {
            return _connection.GetDatabase(this._config.DbNum);
        }
        ...
        public void SetSysPrefix(string customKey)
        {
            this._config.Prefix = customKey;
        }
        ...
        private string AppendPrefix(string oldKey)
        {
            var prefixKey = this._config.Prefix;
            return prefixKey + oldKey;
        }

        private T Do<T>(Func<IDatabase, T> func)
        {
            var database = _connection.GetDatabase(this._config.DbNum);
