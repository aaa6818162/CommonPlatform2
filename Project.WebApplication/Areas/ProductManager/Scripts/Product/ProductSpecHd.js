    ...
    pro.Product.ProductSpecHd = {
        ...
        init: function () {
            ...
            if ($("#BindEntity").val()) {
                ...
                pro.Product.ProductSpecHd.opData.ifBind = true;
                pro.Product.ProductSpecHd.chooseSpec();
                pro.Product.ProductSpecHd.opData.ifBind = false;
            }
        },
        initEvent: function () {
            ...
            $('#skuArea').on('change', "input[name^=GoodsStock_]", function () {
                ...
                JSLINQ(pro.Product.ProductSpecHd.opData.skuList).ForEach(function (skuRow) {
                    ...
                    var goodsStock = parseInt(skuRow.GoodsStock);
                    sumNum += (isNaN(goodsStock) ? 0 : goodsStock);
                });
        ...
        addSkuHtml: function () {
            ...
            JSLINQ(this.opData.skuList).ForEach(function (skuRow) {
                if (skuRow.GoodsPrice == "" && pro.Product.ProductSpecHd.opData.ifBind) {
                    return;
                }
                ...
                var rowHtml = ' <tr style="">\
                           '+ rowSpecHtml + '\
                            ...
                            <td>\
                                <span   name="BtnIsUse_' + skuRow.CombineId + '" >不启用</span> <input type="text" style="display:none" name="IsUse_' + skuRow.CombineId + '" value="' + skuRow.IsUse + '"/>\
                            </td>\
                        </tr>';
                skuAreaHtml += rowHtml;
