...
namespace Project.Service.ProductManager
{
    public class SystemCategoryService
    {
        ...
        public System.Tuple<IList<SystemCategoryEntity>, int> Search(SystemCategoryEntity where, int skipResults, int maxResults)
        {
            ...
            #endregion
            var list = _systemCategoryRepository.Query().Where(expr).OrderBy(p=>p.Sort).ThenByDescending(p => p.PkId).Skip(skipResults).Take(maxResults).ToList();
        ...
        public IList<SystemCategoryEntity> GetList(SystemCategoryEntity where)
        {
            ...
            //  expr = expr.And(p => p.DeletionTime == where.DeletionTime);
            #endregion
            var list = _systemCategoryRepository.Query().Where(expr).OrderBy(p => p.Sort).ThenByDescending(p => p.PkId).ToList();
            return list;
        }
        #endregion
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void AddSystemCategoryAttributeRelation(SystemCategoryAttributeEntity entity)
        {
            _systemCategoryAttributeRepository.Save(entity);
        }
        ...
        /// </summary>
        /// <param name="entity"></param>
        public void AddSystemCategorySpecRelation(SystemCategorySpecEntity entity)
        {
            _systemCategorySpecRepository.Save(entity);
        }
