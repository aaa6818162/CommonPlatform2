...
namespace Project.Service.ProductManager
{
    public class ProductService
    {
        ...
        public Tuple<bool, string> Add(ProductEntity entity)
        {
            ...
            using (var tx = NhTransactionHelper.BeginTransaction())
            {
                try
                {
                    ...
                    var routePath = "";
                    new ProductCategoryService().GerParentProductCategoryRoute(entity.ProductCategoryId, ref routePath);
                    entity.ProductCategoryRoute = routePath;
        ...
        public Tuple<bool, string> Update(ProductEntity entity)
        {
            ...
            orgInfo.IsCommand = newInfo.IsCommand;
            orgInfo.Weight = newInfo.Weight;
            orgInfo.WeightUnit = newInfo.WeightUnit;
            ...
            using (var tx = NhTransactionHelper.BeginTransaction())
            {
                try
                {
                    var routePath = "";
                    new ProductCategoryService().GerParentProductCategoryRoute(entity.ProductCategoryId, ref routePath);
                    orgInfo.ProductCategoryRoute = routePath;
        ...
        public System.Tuple<IList<ProductEntity>, int> Search(ProductEntity where, int skipResults, int maxResults)
        {
            ...
            if (where.ProductCategoryId > 1)
                expr = expr.And(p => p.ProductCategoryRoute.Contains("," + where.ProductCategoryId + ","));
        ...
        {
            var productList = _productRepository.Query().ToList();
            productList.ForEach(p =>
            {
                p.StockNum = p.GoodsEntityList.Sum(x => x.GoodsStock);
