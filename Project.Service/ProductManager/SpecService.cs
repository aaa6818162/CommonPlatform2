    ...
    public class SpecService
    {
       ...
       #region 构造函数
        ...
        public System.Tuple<IList<SpecEntity>, int> Search(SpecEntity where, int skipResults, int maxResults)
        {
            ...
            #endregion
            var list = _specRepository.Query().Where(expr).OrderBy(p=>p.Remark).ThenByDescending(p => p.PkId).Skip(skipResults).Take(maxResults).ToList();
        ...
        public IList<SpecEntity> GetList(SpecEntity where)
        {
             ...
             #region
              ...
              //  expr = expr.And(p => p.ShowType == where.ShowType);
 #endregion
            var list = _specRepository.Query().Where(expr).OrderBy(p=>p.Remark).ThenByDescending(p => p.PkId).ToList();
