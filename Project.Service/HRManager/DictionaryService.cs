...
namespace Project.Service.HRManager
{
    public class DictionaryService
    {
        ...
        /// <param name="pkId">主键</param>
        /// <returns></returns>
        public DictionaryEntity GetModelByKeyCode( System.String KeyCode)
        {
            DictionaryEntity where = new DictionaryEntity();
            //where.ParentKeyCode = "0";
        ...
        public string GetByKeyValue(string keyValue,string parentKeyCode)
        {
            var expr = PredicateBuilder.True<DictionaryEntity>();
            #region
                expr = expr.And(p => p.KeyValue == keyValue);
            expr = expr.And(p => p.ParentKeyCode == parentKeyCode);
            #endregion
            var list = _dictionaryRepository.Query().Where(expr).OrderBy(p => p.PkId).ToList();
            if (list.Any())
            {
