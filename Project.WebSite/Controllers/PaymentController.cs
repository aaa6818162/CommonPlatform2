...
namespace Project.WebSite.Controllers
{
    public class PaymentController : Controller
    {
        ...
        public ActionResult WechatPayNotify()
        {
            try
            {
                LoggerHelper.InfoFormat(LogType.PaymentLogger, "微信支付异步请求开始\r\n");
                var result = WechatPayService.Instance.CheckNotifyData();
                LoggerHelper.InfoFormat(LogType.PaymentLogger, "微信支付异步请求开始：订单号：{0}状态{1}\r\n", result.OrderNo, result.Status);
                if (result.Status)
                {
                    ConfirmOrder(result);
                }
            ...
            {
                LoggerHelper.InfoFormat(LogType.PaymentLogger, "微信支付异步请求异常" + e.Message);
                throw;
            }
        }
